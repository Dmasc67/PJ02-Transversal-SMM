...
if (isset($_POST['id_reserva'])) {
    ...
    // Verificar el estado de la reserva antes de cancelar
    $check_query = "SELECT estado FROM tbl_reservas WHERE id_reserva = :id_reserva";
    $check_stmt = $conexion->prepare($check_query);
    $check_stmt->bindParam(':id_reserva', $id_reserva);
    $check_stmt->execute();
    $reserva = $check_stmt->fetch(PDO::FETCH_ASSOC);

    if ($reserva && $reserva['estado'] !== 'Satisfactoria' && $reserva['estado'] !== 'Cancelada') {
        ...
        // Redirigir de vuelta a reservas.php si no se puede cancelar
        header("Location: reservas.php?error=No se puede cancelar la reserva");
        exit();
    }
} else {
